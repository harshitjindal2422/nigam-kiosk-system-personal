...
const DEATH_PARTICULARS = [
  { id: 1, label: 'Gender' },
  { id: 2, label: 'Date Of Death' },
  { id: 3, label: 'Deceased Name Hindi' },
  { id: 4, label: 'Deceased Name English' },
  { id: 5, label: 'Mother Name Hindi' },
  { id: 6, label: 'Mother Name English' },
  { id: 7, label: 'Father Name Hindi' },
  { id: 8, label: 'Father Name English' },
  { id: 9, label: 'Husband/Wife Name Hindi' },
  { id: 10, label: 'Husband/Wife Name English' },
  { id: 11, label: 'Permanent Address Hindi' },
  { id: 12, label: 'Permanent Address English' },
  { id: 13, label: 'Present Address Hindi' },
...
export default function DeathCorrection() {
  ...
  const [rows, setRows] = useState(
    DEATH_PARTICULARS.reduce((acc, curr) => {
  ...
  const getSelectedCorrections = () => {
    return Object.keys(rows)
      ...
      .map(id => ({
        particular: DEATH_PARTICULARS.find(p => p.id === id).label,
    ...
    <div className="w-full h-full flex flex-col overflow-hidden bg-white select-none relative">
      ...
      {step === 'FORM' && (
        <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} className="flex-1 flex flex-col overflow-hidden relative">
          ...
          {/* DETAILS HEADER (Grey/Black Theme - #4d4d4d) */}
          <div className="bg-[#4d4d4d] px-10 py-6 flex justify-between gap-10 text-white shrink-0">
          ...
          {/* TABLE SECTION */}
          <div className="flex flex-col flex-1 bg-[#5e5e5e] overflow-hidden">
            <div className="grid grid-cols-[80px_80px_1fr_1.2fr_1.2fr] bg-[#000000] text-white font-bold py-3 text-sm text-center shrink-0 shadow-[0_4px_10px_rgba(0,0,0,0.1)] z-10">
      ...
      {tokenRecord && createPortal(
        <div className="hidden print:block w-[80mm] p-[10px] text-black font-mono text-[12px] leading-relaxed">
          ...
          <div className="flex flex-col gap-1 border-b border-dashed border-black pb-2 mb-2">
            ...
            <div><strong>SERVICE:</strong> DEATH CORRECTION</div>
